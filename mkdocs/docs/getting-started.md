...
- `src/`: Fortran source code for the solver, including the conditionally
  compiled variant-specific files.
- `qa/`: Input files for the test problems and scripts for running them.
- `nucdata/`: Nuclear cross-section data files used by the test problems.
- `images/`: Verification figures generated by `verification.py`.
- `docs/`: Documentation build configuration for the Ford API reference.
- `mkdocs/`: Content and configuration for this site.
- `Makefile`: Build rules for compiling the code and running the tests.
- `Dockerfile`: Instructions for building a containerised environment.
...
To build SNES, the diamond-difference variant (the default), open a terminal at the project root and run:

```bash
make
```

To build SNEL, the linear-discontinuous variant, run:
...
Install the Python tooling and set up the git hooks:
...
The hooks run shellcheck and general hygiene checks (whitespace, line endings,
merge-conflict markers, and similar) on each commit.
