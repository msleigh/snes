...
The `compare` script is used to compare the effective multiplication factor ($k_\mathrm{eff}$) between the `snes` and `snel` test outputs. To use it, run:

```bash
./compare
```

This will output the differences in $k_\mathrm{eff}$ values between the two sets of test outputs, helping you identify any discrepancies.
