...
outer_loop: DO
  ...
  scalflux_outer(:,:,0_ik) = scalflux(:,:,0_ik)
  scalflux_outer(:,:,1_ik) = scalflux(:,:,1_ik)
  scalflux_outer(:,:,2_ik) = scalflux(:,:,2_ik)
 ...
 ! Set up fission source and get k_effective
  ...
  ELSE
    scale = 1.0_rk/keff
    source_f(:,:,:) = scale*source_f(:,:,:)
!    source_fr(:,:) = scale*source_fr(:,:)
  END IF
...
group_loop: DO group = 1_ik,numgroups
 ...
 ! If calctype is 2, fisssource,f is not called, therefore source_f is zero
  ...
  source_g(:,2_ik) = source_f(:,group,2_ik) + source_i(:,group)
...
inner_loop: DO
  ...
  IF (inneriter(group) > imaxinner) THEN
    IF (calctype == 1_ik) THEN
      ...
      EXIT inner_loop
    ELSE
      WRITE(*,*) unitname,': Inners not converged - increase parameter IMAXINNERS'
      WRITE(*,*)
      RETURN
    ENDIF
  ENDIF
  ...
  scalflux_inner(:,0_ik) = scalflux(:,group,0_ik)
  scalflux_inner(:,1_ik) = scalflux(:,group,1_ik)
  scalflux_inner(:,2_ik) = scalflux(:,group,2_ik)
  ...
  scalflux(:,group,1_ik) = 0.0_rk
  scalflux(:,group,2_ik) = 0.0_rk
  CALL sweep( &
    & group)
  ...
  conv_loop_inner: DO j = 1,numcells
    error = ABS(1.0-scalflux_inner(j,0_ik)/scalflux(j,group,0_ik))
